 ...
 */
package icu.easyj.spring.boot.autoconfigure.json.fastjson;
 ...
 * @see <a href="https://repo1.maven.org/maven2/com/alibaba/fastjson">fastjson公网发布版本查看页面，可查看特定修复版本</a>
 */
...
public class EasyjCheckFastjsonLoopholeAutoConfiguration implements Ordered {
	...
	public EasyjCheckFastjsonLoopholeAutoConfiguration(LoopholeCheckProperties properties) {
		// 获取 fastjson 的jar信息
		JarInfo jarInfo = JarUtils.getJar("fastjson");

		// 判断版本号：1.2.68及以下版本，且不是特定修复版本的，就存在漏洞
		// 特定漏洞修复版本请查阅：https://repo1.maven.org/maven2/com/alibaba/fastjson
		if (jarInfo != null && EasyjFastjsonBugfixUtils.isLoopholeVersion(jarInfo.getVersion())) {
			...
			logger.warn("为了避免升级导致的兼容性问题，也可升级至低版本的最新特定漏洞修复版本（格式如 '1.2.68.sec10' 的版本号）。");
			logger.warn("fastjson版本号请查看链接（含低版本特定漏洞修复版本）：https://repo1.maven.org/maven2/com/alibaba/fastjson");
			logger.warn("如果您的fastjson版本已经是不存在漏洞的版本，您可以添加配置关闭该警告：easyj.loophole-check.fastjson=false");
			logger.warn("<==");
			logger.warn("");

			if (properties.isNeedThrowException()) {
				throw new ExistLoopholeVersionError("当前fastjson版本存在远程代码执行漏洞，请尽快更新至1.2.69及以上版本，或最新特定漏洞修复版本！");
			}
		}
	}
