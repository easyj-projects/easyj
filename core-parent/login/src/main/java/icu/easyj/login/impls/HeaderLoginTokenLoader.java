 ...
 */
@LoadLevel(name = "header", order = 10)
public class HeaderLoginTokenLoader implements ILoginTokenLoader {

	@Nullable
	@Override
	public String load(HttpServletRequest request, ILoginProperties properties) {
		return request.getHeader(properties.getTokenHeaderName());
	}

	@Override
	public boolean isAllow(ILoginProperties properties) {
		return properties.isAllowHeader();
	}
}
