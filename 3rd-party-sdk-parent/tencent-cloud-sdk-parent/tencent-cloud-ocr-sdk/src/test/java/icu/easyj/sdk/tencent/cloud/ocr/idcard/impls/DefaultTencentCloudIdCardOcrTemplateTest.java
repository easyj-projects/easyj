...
class DefaultTencentCloudIdCardOcrTemplateTest {
	...
	void testIDCardOCR() throws Exception {
		...
		//region case5.1: 正面
		{
			IdCardOcrResponse response = ocrTemplate.idCardOcr(frontIdCardBase64, IdCardOcrAdvanced.ALL);
		...
		//region case5.2: 反面
		{
			IdCardOcrResponse response = ocrTemplate.idCardOcr(backIdCardBase64, IdCardOcrAdvanced.ALL);
		...
		//region case5.3: 正反面
		{
			...
			Assertions.assertEquals(response.getBackIdCardBase64(), response2.getBackIdCardBase64()); // 在存在告警的情况下，有时候，无法获取到身份证裁剪图片
